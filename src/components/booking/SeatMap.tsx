...
import { RenderSeat } from "@/utils/seat-helper"; // Import type mới
...
type Props = {
  seatLayout: RenderSeat[]; // Dữ liệu đã merge
  bookedSeats?: string[]; // List UUID
  heldSeats?: string[]; // List UUID
  chosenSeats: RenderSeat[]; // List Object ghế đang chọn
...
export default function SeatMap({
  ...
  const handleSeatClick = (seat: RenderSeat) => {
    if (seat.type === "EMPTY" || !seat.id) return; // Không có ID (ghế ảo) thì bỏ qua
    ...
    if (isSelected) {
      onToggleSeat(seat); // BookingPage sẽ gọi releaseSeat(...)
      return;
    }

    // Check Booked/Held bằng UUID
    const isBooked =
        ...
        for (let i = 0; i < originalSeats.length; i++) {
          ...
          if (
            ...
            mergedSeats.push({
              ...current,
              isMerged: true,
              // Hiển thị: A1-A2 (Dùng seatNumber từ DB nếu có, không thì fallback)
              displayLabel: `${
          ...
          {rows.map((rowSeats, rowIndex) => (
            <div key={rowIndex} className="flex items-center gap-4">
              ...
              <div className="flex gap-2">
                {rowSeats.map((seat) => {
                  if (!seat.id && seat.type !== "EMPTY") return null; // Skip ghế lỗi
                  ...
                  // Ghế bị người khác giữ (held) nhưng KHÔNG phải ghế mình đang chọn
                  const isHeld = isHeldRaw && !isSelected;

                  // 2. Xác định style cơ bản theo loại ghế
                  const baseStyle =
